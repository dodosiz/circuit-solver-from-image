...
component = rgb2gray(imread([path2 'current.png']));
component = imbinarize(component);

% Iterate through each voltage source
for i = 1:length(currents)
    currCenter = currents(i).center;
    
    % Detect SURF features on each image
    originalPoints = detectSURFFeatures(original,'ROI',[currCenter(1)-80, currCenter(2)-80, 160, 160]);
    ...
    currents(i).orientation = int64(theta_recovered);
